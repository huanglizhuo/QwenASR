...
/// INT4 group quantization (R11-I): dequantized weights must sit within half a
/// quantization step of the original BF16 values, and the NEON matvec must
/// match an exact integer/f64 reference of the same quantized math.
#[test]
fn test_int4_pack_and_matvec() {
    let in_dim = 256; // multiple of 128 (kernel) and 32 (pack)
    let out_dim = 5; // odd: exercises the paired-row loop and the tail row
    let ngroups = in_dim / kernels::INT4_GROUP;
    ...
    let mut next = move || {
        ...
        state ^= state >> 7;
        state ^= state << 17;
        (state >> 40) as f32 / 8388608.0 - 1.0 // [-1, 1)
    };
    let w_f32: Vec<f32> = (0..out_dim * in_dim).map(|_| next() * 0.1).collect();
    let w_bf16: Vec<u16> = w_f32.iter().map(|v| ((v.to_bits() + 0x8000) >> 16) as u16).collect();
    let x_f32: Vec<f32> = (0..in_dim).map(|_| next() * 4.0).collect();

    let (packed, scales) =
        unsafe { kernels::quantize_bf16_weights_to_int4(w_bf16.as_ptr(), out_dim, in_dim) };
    assert_eq!(packed.len(), out_dim * in_dim / 2);
    assert_eq!(scales.len(), out_dim * ngroups);

    // Unpack codes and check the dequantization error bound per group.
    let mut codes = vec![0i32; out_dim * in_dim];
    for row in 0..out_dim {
        for g in 0..ngroups {
            let s = kernels::bf16_to_f32(scales[row * ngroups + g]);
            for j in 0..16 {
                let b = packed[row * in_dim / 2 + g * 16 + j];
                let lo = (b & 0x0F) as i32 - 8;
                let hi = (b >> 4) as i32 - 8;
                codes[row * in_dim + g * 32 + j] = lo;
                codes[row * in_dim + g * 32 + 16 + j] = hi;
                for (c, k) in [(lo, j), (hi, 16 + j)] {
                    let orig = kernels::bf16_to_f32(w_bf16[row * in_dim + g * 32 + k]);
                    let err = (c as f32 * s - orig).abs();
                    assert!(err <= 0.51 * s, "int4 dequant err {} > step/2 {} (row {} k {})",
                        err, 0.51 * s, row, k);
                }
            }
        }
    ...
    {
        use qwen_asr::kernels::neon;
        let mut x_int8 = vec![0i8; in_dim];
        let x_scale = kernels::quantize_into(&mut x_int8, &x_f32);

        // Exact reference of the kernel's math: per-group integer dot (exact
        // in i64), group-scaled and summed in f64, then times x_scale.
        ...
        for row in 0..out_dim {
            ...
            for g in 0..ngroups {
                let mut d = 0i64;
                for k in 0..32 {
                    d += codes[row * in_dim + g * 32 + k] as i64
                        * x_int8[g * 32 + k] as i64;
                }
                acc += d as f64 * kernels::bf16_to_f32(scales[row * ngroups + g]) as f64;
            }
            y_ref[row] = (acc as f32) * x_scale + 0.25 * row as f32;
        }

        let bias: Vec<f32> = (0..out_dim).map(|i| 0.25 * i as f32).collect();
        let mut y = vec![0.0f32; out_dim];
        unsafe {
            neon::matvec_int4_g32(
                &mut y, x_int8.as_ptr(), x_scale,
                packed.as_ptr(), scales.as_ptr(), Some(&bias),
                in_dim, out_dim,
            );
        }
        for row in 0..out_dim {
            let tol = 1e-4 * y_ref[row].abs().max(1.0);
            assert!((y[row] - y_ref[row]).abs() < tol,
                "int4 matvec row {}: {} vs ref {}", row, y[row], y_ref[row]);
        }
    }
}
