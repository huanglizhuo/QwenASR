...
/// `linear`, via `sgemm_nt_pooled`. This covers whether `linear`'s smaller
/// operands and lower slice count (`out_dim = 896` → 7) follow the same vendor
/// split as the conv path. Driven by `QASR_LINEAR_POOLED`.
#[test]
#[ignore = "microbenchmark; run explicitly with --ignored --nocapture"]
...
/// The third site that issues concurrent `cblas_sgemm` calls from the pool:
/// `causal_attention` fans out over query heads with `parallel_for`, and for
/// `seq_q > 1` each worker's
